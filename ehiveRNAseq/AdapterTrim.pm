...
=cut
package ehiveRNAseq::AdapterTrim;
...
sub write_output {
    ...
    my $trim_galore = $self->param_required('trim_galore');
    my $data_dir    = $self->param_required('data_dir'); 
    my $output_dir  = $self->param_required('output_dir')."/trim_galore";
    my $trim_galore = $self->param_required('trim_galore');

    $self->check_dir($output_dir);

    my $cmd1 = "$trim_galore --gzip $data_dir/$data_file1 --o $output_dir";
    my $cmd2 = "$trim_galore --gzip $data_dir/$data_file2 --o $output_dir";
    system($cmd1);
    system($cmd2);

    if ( $? == -1 ){
        print "raw data adapter trimming step failed for $data_file1 & $data_file2: $!\n";
    }
    else {
        printf "raw data adapter trimming step for $data_file1 & $data_file2 exited with value %d\n", $? >> 8;
    }
